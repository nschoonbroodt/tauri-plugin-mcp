...
use interprocess::local_socket::{
    GenericFilePath, GenericNamespaced, Listener as IpcListener, ListenerOptions, Name,
    Stream as IpcStream, ToFsName, ToNsName, prelude::*,
};
use log::{error, info};
...
impl<S: Write + Read> Read for LoggingStream<S> {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        ...
        info!("[TAURI_MCP] Read: {}", String::from_utf8_lossy(&buf[..n]));
        Ok(n)
    }
}
        ...
        *self.running.lock().unwrap() = true;
        ...
        thread::spawn(move || {
            match &socket_type {
                ...
                SocketType::Tcp { host, port } => {
                    info!(
                        "[TAURI_MCP] Listener thread started for TCP socket at {}:{}",
                        host, port
                    );
                }
            }
            ...
            std::panic::set_hook(Box::new(move |panic_info| {
                ...
                if !is_pipe_error {
                    ...
                    info!("[TAURI_MCP] Handled pipe disconnection (normal client disconnect)");
                }
            }));
                            ...
                            match tcp_listener.accept() {
                                Ok((mut stream, addr)) => {
                                    ...
                                    if let Err(e) = stream.set_nonblocking(false) {
                                        error!(
                                            "[TAURI_MCP] Failed to set stream to blocking mode: {}",
                                            e
                                        );
                                        continue;
                                    }
